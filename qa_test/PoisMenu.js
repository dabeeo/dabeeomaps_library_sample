export class PoisMenu {
    constructor(gui, mapData, map, mapContainer) {
        this.gui = gui;
        this.mapData = mapData;
        this.map = map;
        this.mapContainer = mapContainer;
    }
    init() {

        let poisSetting = null; 
        this.mapContainer.addEventListener("floor-changed", (e) => {
            console.log("floor-changed 에 대한 결과값", e.detail);
            const currentFloor = e.detail.id;
            const poiList = pois.reduce((result, cur) => {
                if (currentFloor == cur.floorId) return {...result, [cur.title]:cur.id};
                else return result;
            }, {"":""});
            poisSetting = poisSetting.options(poiList);
            // console.log(poiList);
        });

        const pois = this.mapData.dataPoi.getPois();
        const currentFloor = this.map.context.getCurrentFloor().id;
        const poiList = pois.reduce((result, cur) => {
            if (currentFloor == cur.floorId) return {...result, [cur.title]:cur.id};
            else return result;
        }, {"":""});
        const poiList2 = pois.reduce((result, cur) => {
            if (currentFloor == cur.floorId) result.push(cur.id);
            return result;
        }, [""]);
    // console.log(poiList);

        const map = this.map;
        const set = () => {
            const option = Object.assign({}, setting);
            if (actionSetting.id) {
                option.id = [actionSetting.id];
            }
            console.log(option);
            map.pois.set(option);
        };
        const reset = () => {
            if (actionSetting.id) {
                map.pois.reset(actionSetting.id);
            } else {
                map.pois.reset();
            }
        };
        const hide = () => {
            if (actionSetting.id) {
                map.pois.hide(actionSetting.id);
            } else {
                map.pois.hide();
            }
        };
        const show = () => {
            if (actionSetting.id) {
                map.pois.show(actionSetting.id);
            } else {
                map.pois.show();
            }
        };

        const single_h_to_s = () => {
            const Option ={
                outerColor: 'black',
                innerColor: 'white',
                scale: 3,
                id:poiList2[1]
            }
            console.log(poiList2[1]);
            map.pois.set(Option)
            setTimeout(()=>{
                map.pois.hide(poiList2[1]);
                console.log("hide")
            },1000)
            setTimeout(()=>{
                map.pois.show(poiList2[1]);
                console.log("show")
            },3000);
            setTimeout(()=>{
                map.pois.hide(poiList2[1]);
                console.log("hide")
            },5000)
            setTimeout(()=>{
                map.pois.show([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("show")
            },7000)
            setTimeout(()=>{
                map.pois.hide(poiList2[1]);
                console.log("hide")
            },9000)
            setTimeout(()=>{
                map.pois.show();
                console.log("show")
            },11000)
            setTimeout(()=>{
                map.pois.reset();
            },12000)
        }

        const arr_h_to_s = () => {
            const Option ={
                ids: [poiList2[1],poiList2[2],poiList2[3]] ,
                outerColor: 'black',
                innerColor: 'white',
                scale: 3
            }
            map.pois.set(Option)
            setTimeout(()=>{
                map.pois.hide([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("hide")
            },1000)
            setTimeout(()=>{
                map.pois.show(poiList2[1]);
                console.log("show")
            },3000);
            setTimeout(()=>{
                map.pois.hide([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("hide")
            },5000)
            setTimeout(()=>{
                map.pois.show([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("show")
            },7000)
            setTimeout(()=>{
                map.pois.hide([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("hide")
            },9000)
            setTimeout(()=>{
                map.pois.show();
                console.log("show")
            },11000)
            setTimeout(()=>{
                map.pois.reset();
            },12000)
        }

        const all_h_to_s = () => {
            const Option ={
                // ids: [poiList2[1],poiList2[2],poiList2[3]] ,
                outerColor: 'black',
                innerColor: 'white',
                scale: 3
            }
            map.pois.set(Option)
            setTimeout(()=>{
                map.pois.hide();
                console.log("hide")
            },1000)
            setTimeout(()=>{
                map.pois.show(poiList2[1]);
                console.log("show")
            },3000);
            setTimeout(()=>{
                map.pois.hide();
                console.log("hide")
            },5000)
            setTimeout(()=>{
                map.pois.show([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("show")
            },7000)
            setTimeout(()=>{
                map.pois.hide();
                console.log("hide")
            },9000)
            setTimeout(()=>{
                map.pois.show();
                console.log("show")
            },11000)
            setTimeout(()=>{
                map.pois.reset();
            },12000)
        }

        const single_s_to_h = () => {
            const Option ={
                ids: [poiList2[1]] ,
                outerColor: 'black',
                innerColor: 'white',
                scale: 3
            }
            map.pois.set(Option)
            setTimeout(()=>{
                map.pois.hide();
                console.log("hide")   
            },1000)
            setTimeout(()=>{
                map.pois.show(poiList2[1]);
                console.log("show")
            },3000)
            setTimeout(()=>{
                map.pois.show();
            },4000)
            setTimeout(()=>{
                map.pois.hide([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("hide")  
            },5000)
            setTimeout(()=>{
                map.pois.show(poiList2[1]);
                console.log("show")
            },7000)
            setTimeout(()=>{
                map.pois.show();
            },8000)
            setTimeout(()=>{
                map.pois.hide(poiList2[1]);
                console.log("hide")
            },9000)
            setTimeout(()=>{
                map.pois.show(poiList2[1]);
                console.log("show")
            },11000)
            setTimeout(()=>{
                map.pois.show();
                map.pois.reset();
            },12000)
        }

        const arr_s_to_h = () => {
            const Option ={
                ids: [poiList2[1],poiList2[2],poiList2[3]] ,
                outerColor: 'black',
                innerColor: 'white',
                scale: 3
            }
            map.pois.set(Option)
            setTimeout(()=>{
                map.pois.hide();
                console.log("hide")   
            },1000)
            setTimeout(()=>{
                map.pois.show([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("show")
            },3000)
            setTimeout(()=>{
                map.pois.show();
            },4000)
            setTimeout(()=>{
                map.pois.hide([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("hide")  
            },5000)
            setTimeout(()=>{
                map.pois.show([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("show")
            },7000)
            setTimeout(()=>{
                map.pois.show();
            },8000)
            setTimeout(()=>{
                map.pois.hide(poiList2[1]);
                console.log("hide")
            },9000)
            setTimeout(()=>{
                map.pois.show([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("show")
            },11000)
            setTimeout(()=>{
                map.pois.show();
                map.pois.reset();
            },12000)
        }

        const all_s_to_h = () => {
            const Option ={
                // ids: [poiList2[1]] ,
                outerColor: 'black',
                innerColor: 'white',
                scale: 3
            }
            map.pois.set(Option)
            setTimeout(()=>{
                map.pois.hide();
                console.log("hide")   
            },1000)
            setTimeout(()=>{
                map.pois.show();
                console.log("show")
            },3000)
            setTimeout(()=>{
                map.pois.hide([poiList2[1],poiList2[2],poiList2[3]]);
                console.log("hide")  
            },5000)
            setTimeout(()=>{
                map.pois.show();
                console.log("show")
            },7000)
            setTimeout(()=>{
                map.pois.show();
            },8000)
            setTimeout(()=>{
                map.pois.hide(poiList2[1]);
                console.log("hide")
            },9000)
            setTimeout(()=>{
                map.pois.show();
                console.log("show")
            },11000)
            setTimeout(()=>{
                map.pois.show();
                map.pois.reset();
            },12000)
        }

        const setting = {
            outerColor: '#00ff00',
            innerColor: '#ff00ff',
            scale: 1
        };

        const actionSetting = {
            ids: "",
            set: set,
            reset: reset,
            hide: hide,
            show: show,
            single_h_to_s:single_h_to_s,
            arr_h_to_s:arr_h_to_s,
            all_h_to_s:all_h_to_s,
            single_s_to_h:single_s_to_h,
            arr_s_to_h: arr_s_to_h,
            all_s_to_h:all_s_to_h,
        };
        const menu = this.gui.addFolder("pois menu");
        menu.addColor(setting, "outerColor");
        menu.addColor(setting, "innerColor");
        menu.add(setting, "scale");
        poisSetting = menu.add(actionSetting, "ids", poiList);
        menu.add(actionSetting, "set");
        menu.add(actionSetting, "reset");
        menu.add(actionSetting, "hide");
        menu.add(actionSetting, "show");
        menu.add(actionSetting, "single_h_to_s");
        menu.add(actionSetting, "arr_h_to_s");
        menu.add(actionSetting, "all_h_to_s");
        menu.add(actionSetting, "single_s_to_h");
        menu.add(actionSetting, "arr_s_to_h");
        menu.add(actionSetting, "all_s_to_h");
        return menu;
    }
}
